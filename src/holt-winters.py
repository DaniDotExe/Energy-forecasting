...
def run_hw_pipeline(data_path, output_subdir, seasonal_periods, title_prefix):
    ...
    ax.set_title(f'Predicción Test Holt-Winters ({title_prefix}) - MAPE: {mape:.2f}%')
    ax.legend()
    ax.grid(True, alpha=0.3)
    fig.savefig(os.path.join(out_dir, 'prediccion_test_hw.png'), dpi=300)
    plt.close()
