...
Red Neuronal Recurrente (LSTM) en PyTorch para la predicción de generación solar
...
def run_pipeline(data_path, output_subdir, lookback, title_prefix):
    ...
    # 2. Estandarizar Datos (Z-score normalization)
    ...
    ax.set_title(f'Pérdida por Época ({title_prefix})')
    ax.set_xlabel('Época')
    ...
    model.eval()
    with torch.no_grad():
        y_train_pred_scaled = model(torch.tensor(X_train, dtype=torch.float32)).numpy()
        y_val_pred_scaled = model(torch.tensor(X_val, dtype=torch.float32)).numpy()
        y_test_pred_scaled = model(torch.tensor(X_test, dtype=torch.float32)).numpy()
...
if __name__ == "__main__":
    # Configuración de los dos experimentos
    experimentos = [
        {
        ...
        },
        {
            ...
            'subdir': 'daily',
            'lookback': 30, # Mayor lookback para datos diarios
            'name': 'Datos Diarios'
        }
    ]
