#!/usr/bin/env python3
"""
clima_diario_el_paso_diurno.py
...
diariamente para su uso en un modelo de Red Neuronal Recurrente.
...
def aggregate_daily(df: pd.DataFrame) -> pd.DataFrame:
    """
    Crea una columna 'Fecha' y agrupa los datos calculando el promedio diario
    de las variables climáticas.
    """
    logging.info("Creando columna Fecha y agrupando promedios diarios...")
    
    # Crear columna "Fecha" (ej. "2019-01-01") basándonos en la fecha
    df["Fecha"] = df["time"].dt.date
    
    # Agrupar por "Fecha" y calcular la media de las variables numéricas
    df_daily = df.groupby("Fecha")[["temperature_2m", "wind_speed_10m", "shortwave_radiation"]].mean().reset_index()
...
def main():
    ...
    """
    logging.info("=== Inicio del procesamiento de datos climáticos diarios ===")
    ...
    # 3. Agrupar diariamente
    df_diario = aggregate_daily(df_diurno)
    
    # 4. Formatear y exportar a CSV
    format_and_export(df_diario, OUTPUT_FILE)
